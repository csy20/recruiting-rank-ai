...
from config import DIMENSION_WEIGHTS, SCORING, BEHAVIORAL_WEIGHTS
...
def compute_feature_contributions(
    ...
    tech_dim = dim_scores.get("technical_match", 0.0) or 0.0
    sem_dim = dim_scores.get("semantic_match", 0.0) or 0.0
    career_dim = dim_scores.get("career_quality", 0.0) or 0.0
    beh_dim = dim_scores.get("behavioral", 0.0) or 0.0
    ret_dim = dim_scores.get("retention", 0.0) or 0.0
    risk_dim = dim_scores.get("risk_adjustment", 1.0) or 1.0
    ...
    contributions["tech_core_keywords"] = round(tech_contrib * 0.55, 2)
    contributions["tech_depth_ai"] = round(
        tech_contrib * 0.35 * (features.get("ai_depth", 0.0) or 0.0), 2
    )
    contributions["tech_depth_retrieval"] = round(
        tech_contrib * 0.35 * (features.get("retrieval_depth", 0.0) or 0.0), 2
    )
    contributions["tech_diversity"] = round(
        tech_contrib * 0.15 * (features.get("keyword_diversity", 0.0) or 0.0), 2
    )
    ...
    career_contrib = contributions.get("career_quality", 0.0) or 0.0
    ...
    certs = features.get("certifications", 0.0) or 0.0
    ...
    tfidf = features.get("tfidf_jd_similarity", 0.0) or 0.0
...
def explain_ranking(
    ...
    edu = features.get("education_level", 0.0) or 0.0
    if edu >= 0.8:
        strengths.append("Advanced degree (PhD level)")
    elif edu >= 0.5:
        strengths.append("Graduate degree")

    risk = features.get("risk_score", 0.0) or 0.0
    if risk > 0.3:
    ...
    growth = features.get("growth_rate", 0.5) or 0.5
    if growth > 0.7:
        strengths.append("Strong career progression trajectory")

    ret = dim_scores.get("retention", 0.0) or 0.0
    if ret > 0.7:
    ...
    return {
        ...
        "dimension_scores": {
            k: round(float(v), 4) for k, v in dim_scores.items() if v is not None
        },
